...
prob1 = ODEProblem(bacteria,u0,(0.0,10.0),p)
...
@model function fitbacteria(data, prob1)
    # Priors, P(θ)
    α ~ truncated(Normal(0.7,0.1),0,2)
    β ~ truncated(Normal(0.1,0.1),0,1)
...
pl = StatsPlots.scatter(sol1.t, odedata', label = "Experimento");
...
# display(pl)
plot!(sol1, w=1,color=:red, label = L"N(t)")
